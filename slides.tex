...
% TODO count(Alice x) -> \#
% TODO why "expectation" and "maximization"?
% TODO bei Einführung Zählinformation: p(y) = \prod q_\omega(a|b)
% TODO Folie Illustration Arbeitsweise der RTG
% TODO bei Einführung RTG: alle $y$ -> alle $y\in Y$
...
\begin{frame}\frametitle{Versteckte Information für Hidden-Markov-Modell}
 \begin{center}\begin{tikzpicture}
  ...
  \begin{scope}[note/.style={color=white}]
   ...
   \node[align=left,note,node font=\scriptsize] at (-1.5,-6) {
    Knotenbeschriftung:\\ $c = (a,b)$
   };
  \end{scope}

  \node[rectangle] (spacing) at (6.85,-6.15) {};
 \end{tikzpicture}\end{center}
\end{frame}

\newcommand\gstate[2]{{\color{blue}(#1,#2)}}
\newcommand\cevent[3]{{\color{red!90!black}\bigl(#1,(#2,#3)\bigr)}}
\begin{frame}\frametitle{Beispiel: Ableitung aus der Baumgrammatik $K$}
 \begin{center}\begin{tikzpicture}[every node/.style={rectangle,draw,node font=\scriptsize,minimum width=2.5cm}]
  \tikzstyle{gstate}=[color=blue]
  \tikzstyle{cevent}=[color=red!90!black]
  \node<1>  [gstate] (r01) at (+0.0,+0) { \strut$(\#,T)$ };
  \node<2-> [cevent] (r01) at (+0.0,+0) { \strut$\bigl(\text{noun},(\#,T)\bigr)$ };
  \node<2>  [gstate] (r11) at (-3.0,-1) { \strut$(\text{noun},E)$ };
  \node<3-> [cevent] (r11) at (-3.0,-1) { \strut$\bigl(\text{Alice},(\text{noun},E)\bigr)$ };
  \node<2-3>[gstate] (r12) at (+0.0,-1) { \strut$(\text{noun},T)$ };
  \node<4-> [cevent] (r12) at (+0.0,-1) { \strut$\bigl(\text{verb},(\text{noun},T)\bigr)$ };
  \node<4>  [gstate] (r21) at (-3.0,-2) { \strut$(\text{verb},E)$ };
  \node<5-> [cevent] (r21) at (-3.0,-2) { \strut$\bigl(\text{sees},(\text{verb},E)\bigr)$ };
  \node<4-5>[gstate] (r22) at (+0.0,-2) { \strut$(\text{verb},T)$ };
  \node<6-> [cevent] (r22) at (+0.0,-2) { \strut$\bigl(\#,(\text{verb},T)\bigr)$ };
 ...
 \end{tikzpicture}\end{center}
 \begin{itemize}
  \item Zustände: $\gstate qT$ für $q\in Q\cup\{\#\}$ und $(q,e)$ für $q\in Q$
  \item Startzustand: $\gstate\#T$; Regeln:
   ...
   \end{align*}
  \item $H(x)$: Zustände enthalten noch nicht generierte Wörter
 \end{itemize}
\end{frame}
