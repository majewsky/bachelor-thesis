...
\begin{frame}\frametitle{Versteckte Information für Hidden-Markov-Modell}
 \begin{center}\begin{tikzpicture}
  ...
  \begin{scope}[every node/.style={rectangle,draw,node font=\scriptsize,minimum width=2.5cm}]
   \node (r00) at (+5.0,-3.0) { $(\#,T) \to \text{noun}$ };
   \node (r10) at (+5.0,-4.0) { $(\text{noun},T) \to \text{verb}$ };
   \node (r11) at (+2.0,-4.0) { $(\text{noun},E) \to \text{Alice}$ };
   \node (r20) at (+5.0,-5.0) { $(\text{verb},T) \to \text{noun}$ };
   \node (r21) at (+2.0,-5.0) { $(\text{verb},E) \to \text{sees}$ };
   \node (r30) at (+5.0,-6.0) { $(\text{noun},T) \to \#$ };
   \node (r31) at (+2.0,-6.0) { $(\text{noun},E) \to \text{Bob}$ };
  ...
  \begin{scope}[note/.style={color=white},every node/.style={node font=\tiny,note}]
   \only<2>{\tikzset{note/.style={color=black}}}
