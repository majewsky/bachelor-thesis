...
When training a language model, most of the time, not all required data is
...
It is usually intractable to evaluate this $\argmax$ due to the large size of
...
Second, even if this lemma is not applicable, the set of countable events $C$ is
...
\begin{definition}
 ...
 is a triple $\ug = (Q,q_0,R)$ where $Q$ is a nonempty alphabet (of \emph{grammar states}),
 $q_0\in Q$ is an \emph{initial state}, and $R\subset Q^*\times\Sigma\times Q$ is a
 finite ranked alphabet (of \emph{rules}) such that
 \[
  \forall \rho = \mbig\kla{(q_1,\ldots,q_k),\sigma,q}\isin R\colon \rk(\rho) = k.
  \qedhere
 \]
\end{definition}
...
\begin{definition}\label{def:02-past}
 ...
 $q\in Q$,
 \[
  q \in D^q \quad\text{and}\quad \forall \rho=\mbig\kla{q\to\sigma(q_1,\ldots,q_k)}\isin R,d_1\in D^{q_1},\ldots,d_k\in D^{q_k}\colon \rho(d_1,\ldots,d_k) \in D^q.
 \]
 An \emph{abstract syntax tree} is a partial abstract syntax tree $d\in
 D^{q_0}(\ug)$ that does not have any labels from $Q$.
\end{definition}

An RTG generates a language (i.~e.,~a countable set) of trees. Trees are
...
\begin{definition}
 ...
 \[
  \mbig\kla{\pi(d)}(p) := \mbig\abs{\mbig\brc{w\in\pos(d): d(w)=p}}. \qedhere
 \]
\end{definition}
