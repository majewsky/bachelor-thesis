...
% TODO: X-corpus "c" -> "h" everywhere (to avoid collision with c\in C)
...
\begin{definition}
 Let $c$ be a $A\times B$-corpus $c$. For any $a\in A$ and $b\in B$, we define
 the $A$-corpus $c_b$ by $c_b(a) := c(a,b)$, and the \emph{empirical
 conditional probability distribution} $\tilde c\in \um(A|B)$ by
 \[
  \tilde c(a|b) := \frac{c(a,b)}{\sum_{a'\in A} c(a',b)} = \frac{c_b(a)}{\abs{c_b}} = \widetilde{c_b}(a). \qedhere
 \]
\end{definition}
...
Second, even if lemma is not applicable, the set of countable events $C$ is
...
\begin{definition}\label{02:def-io-info}
 ...
 \[
  p'_\omega(\rho) := \begin{cases}
   q_\omega(a|b) & \text{if } \rho=q\to(a,b)(q_1,\ldots,q_k)\in R(K),
   \\ 0 & \text{otherwise}.
  \end{cases}\qedhere
 \]
\end{definition}
...
the sum of all these probabilities is the inside weight $\beta_x(q_0)$.
...
$q$ (described by the outside weight $\alpha_x(q)$), the rule $\rho$ that
