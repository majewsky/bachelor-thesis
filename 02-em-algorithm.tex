...
\begin{definition}
 Let $c$ be an $X$-corpus. Its \emph{size} is defined as
 \[
 ...
 \]
 The corpus is called \emph{empty} if $\abs c = 0$, and \emph{finite} if
 $\abs c<\infty$.
\end{definition}
...
\begin{definition}
 ...
 mapping
 \[
  \overline p\colon \mnorm\kla{\zr_{\geq0}}^X \to \um(X),
  \quad
...
\begin{definition}
 ...
 \[
  \argmax_{a\in A} f(a) := \brc{a\in A|\forall a'\in A: f(a) \geq f(a')}.
 \]
\end{definition}
...
\begin{definition}
 ...
 \[
  ...
  \quad
  c \mapsto \argmax_{\omega\in\Omega} p_\omega(c).
 \]
\end{definition}
...
For the remainder, let\label{02-basic-requirements}
...
local maximum or saddle point of likelihood. The iteration is therefore usually
...
To simply notation, we define $p(a|b) := \mbig\kla{p(b)}(a)$.
...
observation $x$.\footnote{Most instances of $\lambda$ use only integer
...
\begin{definition}
 ...
 $\varkappa^\flat\colon\Omega\to\zr_{\geq0}^{Y\times X}$ is given by
 \[
...
\begin{definition}
 ...
 \[
  S\cdot T := \brc{\sigma\tau|\sigma\in S, \tau\in T}.
 \]
\end{definition}

In this regard, the alphabet $\Sigma$ is itself a set of words since letters
...
at leaves. For example, for $\Sigma = \brc{\sigma_1,\sigma_2}$ and
$V=\brc{v_1,v_2}$:
...
We refer to positions in the tree using \emph{Gorn notation}.
...
Positions are words from $\zn^*$. As an illustration, in the figure below, each
node of the left tree is labeled with its position in the tree. The right tree
illustrates some further notation for trees, which is defined below.
...
\begin{definition}
 ...
 \[
  % TODO: need formal definition for "number of children of w in t"
  T_R(V) := \mbig\brc{t \in U_R(V) \dmiddle| \forall w\in\pos(t): t(w)\in R\Rightarrow \rk\mbig\kla{t(w)} = \text{number of children of $w$ in $t$}}.
 \]
\end{definition}

In other words, the number of children of each node in $t$ labeled with a
symbol from $R$ must be equal to the rank of its label.
...
We will write $\mbig\kla{(q_1,\ldots,q_k),\sigma,q}\in R$ as $q\to\sigma(q_1,\ldots,q_k)$.
...
In the general case, \cite[pp.~6]{bucstuvog15} show that $\beta$ is the least fixpoint of the mapping $F: (\zr_{\geq0}\cup\brc\infty)^Q \to (\zr_{\geq0}\cup\brc\infty)^Q$, given by
\[
...
\begin{definition}
 ...
 \begin{itemize}\setlength\itemsep{-0.3em}
  ...
  \item $H\colon X_{\not\bot}\to\ur(C)$ assigns an unambiguous RTG to every observation such that $\lang{H(x)} = \pi_1^{-1}(x)$.
 \end{itemize}
...
to this observation is generated by the PRTG $H(x)$.
