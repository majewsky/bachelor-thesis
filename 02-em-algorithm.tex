...
\begin{definition}
 ...
 congruent for language models by letting $\pi_1 := \pi$ and $\pi_2(y) :=
 \emptyset\in Z$.} $\mu = (q,\pi,K,H)$ such that
 \begin{itemize}\setlength\itemsep{-0.3em}
  ...
  \item $H:X_{\not\bot}\to\ur(C)$ assigns a PRTG to every observation such that $\lang{H(x)} = \pi^{-1}(x)$.
 \end{itemize}
\end{definition}
...
\begin{definition}
 Let $\mu=(q,\pi,K,H)$ be an IO information. The \emph{induced counting
...
\cite[p.~15]{bucstuvog15} Using it, we can construct a suitable complete-data corpus:
\[
...
trees, is $\pi^{-1}(x)$, the set of all hidden informations that lead to the
...
\begin{enumerate}
 ...
 \item inside-outside informations $\mu=(q,\pi,K,H)$
\end{enumerate}
...
and of the IO information $\mu=(q,\pi,K,H)$, the only part that depends on
