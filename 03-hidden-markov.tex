...
\]
if we define $x_{n+1}\cdots x_n := \eps$ as a corner case. Substitution of $x'$ by $x_i\cdots x_n$ in the equation for $\beta_x\doublekla{T,q,x'}$ above leads to
\[
 \beta_x\doublekla{T,q,x_i\cdots x_n} = \sum_{q'\in Q} t(q'|q) \cdot e(x_i|q') \cdot \beta_x\doublekla{T,q',x_{i+1}\cdots x_n}.
\]
The way in which $\beta_x\doublekla{T,q,x_i\cdots x_n}$ is expressed in terms of $\beta_x\doublekla{T,q,x_{i+1}\cdots x_n}$ is the same way in which the backward weights depend on each other in~\eqref{eq:03-S_v} on page~\pageref{eq:03-S_v}. Because
\[
 \beta_x\doublekla{T,q,x_{n+1}\cdots x_n} = \beta_x\doublekla{T,q,\eps} = t(\#|q) = S_x(n,q)
\]
for all $q\in Q$, the inside weights recover the backward weights, i.~e.,
\[
...
\]
by~\eqref{eq:03-p-by-s}. The statement $\beta_x\doublekla{T,\#,x} = P(x)$ also holds for $x=\eps$ because
\label{eq:03-beta-eps}\[
 \beta_\eps\doublekla{T,\#,\eps} = t(\#|\#) = P(\eps).
\]
...
weights bear no relevance for the following.
...
Otherwise, let $x=x_1\cdots x_n\isin V^+$ such that, as before, every
$x'\in\suff(x)$ has the form $x'=x_i\cdots x_n$ for some
$i\in\brc{1,\ldots,n}$. Other than $\alpha_x\doublekla{T,\#,x}$, we have nonzero
outside weights
\begin{align*}
 \alpha_x\doublekla{T,q,x_{i+1}\cdots x_n} &= \sum_{q'\in Q_\#} \alpha_x\doublekla{T,q',x_i\cdots x_n} \cdot \underbrace{q_\omega\mbig\kla{q\npipe(q',T)}}_{=t(q|q')} \cdot \underbrace{\beta_x\doublekla{E,q,x_i\cdots x_n}}_{=e(x_i|q)} \\
 \text{and }
 \alpha_x\doublekla{E,q,x_i\cdots x_n}     &= \sum_{q'\in Q_\#} \alpha_x\doublekla{T,q',x_i\cdots x_n} \cdot \underbrace{q_\omega\mbig\kla{q\npipe(q',T)}}_{=t(q|q')} \cdot \underbrace{\beta_x\doublekla{T,q,x_{i+1}\cdots x_n}}_{=S_x(i,q)},
\end{align*}
...
\begin{align*}
 \alpha_x\doublekla{T,q,x_1\cdots x_n} &= T_x(1,q), \\
 \alpha_x\doublekla{T,q,x_i\cdots x_n} &= \sum_{q'\in Q} \alpha_x\doublekla{T,q',x_i\cdots x_n} \cdot t(q|q') \cdot e(x_i|q) &\text{for } i\in\brc{2,\ldots,n}
\end{align*}
for any $q\in Q$. Comparison of this set of equations with~\eqref{eq:03-T_v-base} and~\eqref{eq:03-T_v} shows that the outside weights recover the inside weights, i.~e.,
\[
...
\subsection{Complete-data corpus}\label{sect:03-complete-data}
...
\begin{equation}\label{eq:03-chi}
 \chi_{\omega,x}(a,b) = \underbrace{\beta_x\doublekla{T,\#,x}^{-1}}_{=P(x)^{-1}} \cdot \sum_{(q\to(a,b)(q_1,\ldots,q_k))\in R_x} \alpha_x(q) \cdot q_\omega(a|b) \cdot \beta_x(q_1) \cdots \beta_x(q_k).
\end{equation}

For $x=\eps$, we only have one nonzero outside weight, $\alpha_\eps\doublekla{T,\#,\eps}
= 1$, and only one rule that applies to this state, $(T,\#,\eps) \to
\mbig\kla{\#,(\#,T)}$, from which follows that
\[
 \chi_{\omega,\eps}\mbig\kla{\#,(\#,T)} = \underbrace{P(x)^{-1}}_{=t(\#|\#)^{-1}} \cdot \underbrace{\alpha_x\doublekla{T,\#,\eps}}_{=1} \cdot \underbrace{q_\omega\mbig\kla{\#\npipe(\#,T)}}_{=t(\#|\#)} = 1
\]
and all other $\chi_{\omega,\eps}(a,b)$ vanish. Since $(T,\#,\eps)$ is
unreachable in all $H(x)$ with $x\neq\eps$, we have $\alpha_x\doublekla{T,\#,\eps} = 0$
...
\begin{align*}
 ...
 &= P(x)^{-1} \cdot \sum_{x'\in\suff(x)} \alpha_x\doublekla{T,q,x'} \cdot \underbrace{q_\omega\mbig\kla{q'\npipe(q,T)}}_{=t(q'|q)} \cdot \beta_x\doublekla{E,q',x'} \cdot \beta_x\mBig\kla{\mbig\kla{T,q,\cdr(x')}}. \\
 \intertext{To simplify this expression, we once more rewrite $\sum_{x'\in\suff(x)}$ into $\sum_{i=1}^n$ such that $x' = x_i\cdots x_n$.}
 \chi_{\omega,x}\mbig\kla{q',(q,T)}
 &= P(x)^{-1} \cdot \sum_{i=1}^n \,\underbrace{\alpha_x\doublekla{T,q,x_i\cdots x_n}}_{=\,0\text{ for }i\,=\,1} \cdot t(q'|q) \cdot \underbrace{\beta_x\doublekla{E,q',x_i\cdots x_n}}_{=e(x_i|q)} \cdot \underbrace{\beta_x\mBig\kla{\mbig\kla{T,q,x_{i+1}\cdots x_n}}}_{=S_x(i,q)} \\
