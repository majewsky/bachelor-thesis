...
\begin{definition}
 ...
 \emph{set of suffixes}, including $x$ itself and the empty sentence, i.~e.,
 \[
  \suff(x) := \begin{cases}
...
\begin{align*}
 \suff(bbcb) &= \brc{bbcb, bcb, cb, b, \eps}, &
 \car(bbcb) &= b, &
 \cdr(bbcb) &= bcb.
\end{align*}
Using this operator, we define, for any $x\in X_{\not\bot} = V^*$,
\[
 H(x) := \mbig\brc{Q_x,(T,\#,x),R_x} \quad\text{where}\quad Q_x = \brc{E,T} \times Q_\# \times \suff(x)
\]
