...
Let $\omega = (t,e)\in\Omega$. Since the definition of the complete-data corpus
$c\!\dangle{\omega,\mu}$ is based on on the family
$(\chi_{\omega,x}|\omega\in\Omega,x\in X_{\not\bot})$ of $C$-corpora, which in
turn is defined in terms of the inside weights $\beta$ and outside weights
$\alpha$ of the PRTG $\mbig\kla{H(x),p_\omega'}$ for all $x\in
X_{\not\bot}=V^*$. We therefore have to derive expressions for all these
components.

Let $x\in X_{\not\bot}=V^*$. The $H(x)$ form an entire family of PRTG. We are
therefore going to disambiguate their inside weights $\beta$ as $\beta_x$, and
their outside weights $\alpha$ as $\alpha_x$. Recall that inside weights can,
in general, be calculated by
\begin{repeatequation}{eq:02-beta}
 \beta(q) = \sum_{q_1,\ldots,q_k,\sigma} p\mbig\kla{q\to\sigma(q_1,\ldots,q_k)} \cdot \beta(q_1) \cdots \beta(q_k),
\end{repeatequation}
where $q,q_1,\ldots,q_k$ are grammar states of the PRTG in question, and
$\sigma$ is drawn from the grammar's alphabet, and $p$ is the probability distribution of the PRTG. Instantiating this equation for $\mbig\kla{H(x),p_\omega'}$ leads to
\[
...
\]
if we admit $x_{n+1}\cdots x_n := \eps$ as a corner case. Putting all this into
the previous equation, we have
\[
 \beta_x(T,q,x_i\cdots x_n) = \sum_{q'\in Q} t(q'|q) \cdot e(x_i|q') \cdot \beta_x(T,q',x_{i+1}\cdots x_n).
\]
By comparing with~\eqref{eq:03-S_v}, we see that the inside weights recover the backward weights $S_x$ for $q\neq\eps$, i.~e.,
\[
 \beta_x(T,q,x_i\cdots x_n) = S_x(i-1,q).
\]
Furthermore, for the initial grammar state $(T,\#,x)$, we have
\[
 \beta_x(T,\#,x) = \beta_x(T,\#,x_n) = \sum_{q'\in Q} t(q'|q) \cdot e(x_1|q') \cdot \underbrace{\beta_x(T,q',x_2\cdots x_n)}_{=S_x(1,q)} = P(x)
\]
...
For quick referencing, the final equations for all $\beta_x(q)$, as derived in
this section, can be found in Table~\ref{table:alpha-beta} on
page~\pageref{table:alpha-beta}.

\subsection{Outside weights}

As shown before, the outside weights form a linear equation system according to
\begin{repeatequation}{eq:02-alpha}
 \alpha(q) = \delta_{q_0}^q + \sum_{q',q_1,\ldots,q_k,\sigma,m:\,q_m=q} \alpha(q') \cdot p\mbig\kla{q'\to\sigma(q_1,\ldots,q_k)} \cdot \prod_{l\neq m} \beta(q_l).
\end{repeatequation}
Instantiation of this equation for the PRTG $\mbig\kla{H(x),p_\omega'}$ yields
\[
...
\[
 \alpha_x(T,q,x_i\cdots x_n) = T_x(i-1,q)
\]
for all $i\in\brc{1,\ldots,n}$ and $q\in Q$. Inserting these findings into the
outside weights of the states $(E,q,x_i\cdots x_n)$, we get
\[
...
\end{align*}
for $i>1$. By comparing these expressions with~\eqref{eq:03-T_v} once more, we see that both cases can be summarized as
\[
 \alpha_x(E,q,x_i\cdots x_n) = \frac{T_x(i,q) \cdot S_x(i,q)}{e(x_i|q)}.
\]

For quick referencing, the final equations for all $\alpha_x(q)$, as derived in
this section, can be found in Table~\ref{table:alpha-beta} on
page~\pageref{table:alpha-beta}.
...
\begin{table}[p!]
 ...
 \begin{tabular}{|c|c||c|c|}%
  ...
  \\\hhline{|=|=#=|=|}\mystrut
  $(T,\#,\eps)$ & for $x=\eps$ (initial) & $t(\#|\#)$ & $1$ for $x=\eps$
  \\\hline\mystrut
  $(T,\#,x)$ & yes (initial) & $P(x)$ & $1$
  \\\hline\mystrut
  $(T,\#,x_i\cdots x_n)$ & no & 0 & 0
  \\\hline\mystrut
  $(T,q,\eps)$ & for $x\neq\eps$ & $t(\#|q)$ & $T_x(n,q)$
  \\\hline\mystrut
  $(T,q,x)$ & no & 0 & 0
 ...
 \caption{
  %TODO: corpora
  \label{table:alpha-beta}
  Overview of all inside weights and outside weights for the IO information $\mu_\uh$ as introduced in this chapter, for any fixed $\omega = (t,e)\in \Omega$.
  \\[1ex]
  Throughout the table, statements apply for all $q\in Q$ and $x\in V^*$. Exception: In rows that reference $x$ in the leftmost column, $x=\eps$ is forbidden, and statements apply for all $x=x_1\cdots x_n\in V^+$ and $i\in\{1,\ldots,n\}$.
 }
\end{table}
