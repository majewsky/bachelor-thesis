...
\[
 H(x) := \mbig\brc{Q_x,(\#,0,T),R_x} \quad\text{where}\quad Q_x = \brc{E,T} \times Q_\# \times \brc{0,\ldots,n}
\]
...
$x_i$ according to the desired observation $x$. Using the same reasoning as above for $K$, we see that each $H(x)$ is deterministic and, therefore, unambiguous.

\begin{lemma}\label{lemma:03-language-partition}
 With the definitions shown above,
 \[
  \forall x\in X_{\not\bot}, y\in X_{\not\bot}: \mBig\kla{y\in \mbig\lang{H(x)} \Leftrightarrow x = \pi_1(y)}.
 \]
\end{lemma}

The proof for this lemma can be found in
Appendix~\ref{app:proof-language-partition}. Its meaning is that every hidden
information $y\in Y_{\not\bot}$ can be generated by the RTG $H(x)$ where $x =
\pi_1(y)$, but not by any $H(x')$ for $x'\neq x$. Therefore, $\lang{H(x)}$
contains precisely those $y\in Y_{\not\bot}$ for which $\pi_1(y) = x$. In other
words,
\[
 \forall x\in X_{\not\bot}: \lang{H(x)} = \pi_1^{-1}(x)
\]
...
{\color{red}TODO: proof for partitioning lemma; $\alpha$, $\beta$, $\chi$, $c\dangle{\omega,\mu}$, $\stepmap\mu_\mathrm{io}$}
